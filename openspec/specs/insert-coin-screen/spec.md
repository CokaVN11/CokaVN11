...
### Requirement: Title screen shows PLAYER labels, Coka Portfolio title, and INSERT COIN CTA

The title screen SHALL display "PLAYER 1 / COKA" and "PLAYER 2 / CLIENT" at `--text-label` and 16px respectively, "Coka Portfolio" at `--text-hero` in `--accent` color, and "► INSERT COIN ◄" blinking at 600ms using class `insert-coin-blink` at `--text-name` in `--accent`. See `docs/screens/insert-coin.md`.
